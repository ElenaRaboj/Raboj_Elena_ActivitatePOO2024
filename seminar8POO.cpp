...
private:
	Magazin magazin; // e deja initializat in lista, a fost deja creat, e obiect
	Cofetarie *cofetarie; // l a facut profu, e pointer
	int nrMasiniParcate;
...
public:
	...
	ComplexComercial(Magazin mag, Cofetarie* cof, int nrMasini, Masina* masini) {
		...
		//this->cofetarie = cof; //copiere shellow ca e pointer
		this->cofetarie = new Cofetarie(*cof);//obtin ob prin deferentiere
		this->nrMasiniParcate = nrMasini;
		this->masiniParcate = new Masina[nrMasini];//alocare spatiu;
		for (int i = 0;i < nrMasini;i++)
		{
			this->masiniParcate[i] = masini[i];// se apeleaza operatorul egal din clasa masina
		}// in noua clasa, folosim  ce avem in clasele anterioare
	...
	ComplexComercial(const ComplexComercial& cc)
	{
		...
		//this->cofetarie = cof; //copiere shellow ca e pointer
		this->cofetarie = new Cofetarie(*(cc.cofetarie));//obtin ob prin deferentiere
		this->nrMasiniParcate = cc.nrMasiniParcate;
		this->masiniParcate = new Masina[cc.nrMasiniParcate];//alocare spatiu;
	...
	~ComplexComercial() {
		if (this->cofetarie) {
			delete this->cofetarie;//am alocat un sg obiect si nu am[]
		}
...
//operat0r <<
	...
	friend ostream& operator<<(ostream& o, const ComplexComercial& cc) {
		o << "magazinul din complexul: " << endl;
		o << cc.magazin << endl; //se apeleaza operatorul <<din clasa magazin 
		o << "cofetaria din complex: " << endl;
		o << *cc.cofetarie << endl;//se apeleaza op <<din cof;
		o << " avem " << cc.nrMasiniParcate << "masini parcate in complex: ";
		for (int i = 0;i < cc.nrMasiniParcate;i++)
		{
	...
	ComplexComercial operator--() { //prefixata, nu primim parametru
		...
		delete[] this->masiniParcate; // se apeleaza destructorul din clasa masina, de cate masini aveam inainte, adica 5
		this->masiniParcate = aux; // se apeleaza op= pentru pointeri
	}
		return *this;
	}
	...
	{
		ComplexComercial cc = *this; //stare initiala salvata, se apeleaza constructor de copiere din complex comercial
		--*(this);
		return cc; //starea nemodificata
	}
...
void main() {
	...
	cc1=cc2--; //sa stearga ultima masina; returneazza starea initiala
