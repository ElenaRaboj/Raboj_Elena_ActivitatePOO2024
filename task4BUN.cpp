...
istream& operator>>(istream& input, Floare fo) {
	...
	char buffer[50];
	fo.culoare = new char[strlen(buffer) + 1];
	strcpy_s(fo.culoare, strlen(buffer) + 1, buffer);
	cout << "culoarea:";
	input >> fo.culoare;
	cout << "nume Floare:";
	input >> fo.numeFloare;
...
istream& operator>>(istream& input, Clienti cli) {
	...
	cli.adresa = new char[strlen(buffer) + 1];
	strcpy_s(cli.adresa, strlen(buffer) + 1, buffer);
	cout << "adresa:";
	input >> cli.adresa;
	cout << "numele:";
	cin >> cli.nume;
