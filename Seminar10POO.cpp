...
public:
	...
	//metoda de scriere in fisier binar
	void scriereFisierBinar(fstream& fisier) {
		
		fisier.write((char*)&nrRoti, sizeof(nrRoti));
		for (int i = 0;i < nrRoti;i++) {
			int lungime = producatori[i].length();//salvez lungimea
...
int main() {
	...
	//prod[2] = "hankook";
	//prod[3] = "bridgestone";
	//Masina masina(4, "dacia", 500.0f, 2020);
	//masina.setNrRoti(nr_Roti, prod);
	////cream obiect binar.fstream
